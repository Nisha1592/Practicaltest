...
fun setImageUri(view: ImageView, userImage: Uri?) {
    /*when (userImage.toString()) {
        null -> Unit
        "" -> view.setBackgroundResource(R.drawable.ic_baseline_person_24)
        else -> view.setImageURI(userImage)
    }*/
...
fun TextView.bindEpochTimeMsToDate(epochTimeMs: Long) {
    if (epochTimeMs > 0) {
        ...
        val pat = SimpleDateFormat().toLocalizedPattern().replace(
                "\\W?[$replacePattern]+\\W?".toRegex(),
                " "
        )
        val formatter = SimpleDateFormat(pat, Locale.getDefault())
